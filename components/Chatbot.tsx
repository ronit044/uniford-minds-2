...
export default function Chatbot() {
  ...
  return (
    <div className={`fixed bottom-4 left-4 ${isMinimized ? 'w-16 h-16 rounded-full' : 'w-full max-w-sm h-[calc(100vh-4rem)]'} ${isMinimized ? 'left-0' : 'left-4'} transition-all duration-300`}>
      {!isMinimized && (
        <div className="bg-white p-4 rounded-lg shadow-lg h-full overflow-hidden">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold text-blue-500">Chatbot</h1>
            <button 
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {topics.map((topic) => (
                  <button
                    ...
                    className="bg-white text-blue-500 border border-blue-500 p-2 rounded-3xl hover:bg-blue-500 hover:text-white"
                  >
                    {topic.title}
                  </button>
                ))}
      ...
      {isMinimized && (
        <div className="flex items-center justify-center rounded-full h-full w-full bg-blue-500">
          <button 
